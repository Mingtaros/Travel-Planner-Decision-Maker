...
#==================
# knowledge bases for multi agents
...
def create_variable_agent():
    # Create the agent that generates the multi objective function weights (for alns) based on the traveller's query
    # Budget priority weight
    # Time priority weight
    # Satisfaction priority weight
...
def create_intent_agent():
    # Create the Intent Agent for the agentic workflow, aka Supervisor Agent that determines how the query will be routed
    intent_agent = Agent(
...
# def create_hawker_agent(model_id="deepseek-r1-distill-llama-70b", batch_no=0, debug_mode=True):
    ...
    hawker_agent = Agent(
        ...
        instructions=[
            ...
            "- 'duration': duration of visit in minutes, which is typically around 60 minutes. However, always estimate the amount of duration required based on the tourist's personality and preference.",
...
# def create_attraction_agent(model_id="deepseek-r1-distill-llama-70b", batch_no=0, debug_mode=True):
    ...
    attraction_agent = Agent(
        ...
        role="Search the internal knowledge base and web for information",
        instructions=[
            "IMPORTANT: Provide details on all of the attractions from the internal knowledge base.",
            ...
            "- 'duration': duration of visit in minutes, which is typically around 120 minutes. However, always estimate the amount of duration required based on the tourist's personality and preference.",
